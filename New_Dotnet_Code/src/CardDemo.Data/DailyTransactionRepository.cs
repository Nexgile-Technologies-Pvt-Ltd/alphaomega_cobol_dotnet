...
/// <summary>
/// Relational repository for the DAILY_TRANSACTION table (copybook CVTRA06Y, sequential input). Primary
/// key <c>tran_id</c>; same columns as TRANSACTION. Exposes the VSAM-semantics operations over
/// <see cref="DailyTransaction"/>. The <c>desc</c> column is a SQL keyword and is quoted in every statement.
/// </summary>
public sealed class DailyTransactionRepository : RepositoryBase
{
    private const string Table = "DAILY_TRANSACTION";
    ...
    public DailyTransactionRepository(RelationalDb db) : base(db) { }
    public DailyTransactionRepository(SqliteConnection connection) : base(connection) { }

    /// <summary>Random read by primary key. Returns '00' or '23'.</summary>
    public string ReadByKey(string tranId, out DailyTransaction? transaction)
    {
    ...
    public string Insert(DailyTransaction x)
    {
    ...
    public string Update(DailyTransaction x)
    {
        using SqliteCommand c = Cmd(
    ...
    public string ReadNext(out DailyTransaction? transaction)
    ...
    public string ReadPrevious(out DailyTransaction? transaction)
    ...
    public IEnumerable<DailyTransaction> ReadAll()
    {
    ...
    /// <summary>Maps the current reader row to a <see cref="DailyTransaction"/>.</summary>
    private static DailyTransaction Map(SqliteDataReader rd) => new()
    {
    ...
    /// <summary>Binds a <see cref="DailyTransaction"/> onto the named parameters of an insert/update command.</summary>
    private static void Bind(SqliteCommand c, DailyTransaction x)
    {
