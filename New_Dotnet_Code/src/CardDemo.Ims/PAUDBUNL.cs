...
/// layout <see cref="Paudblod"/> reads back, so an unload then load round-trips.</para>
...
public sealed class Paudbunl
{
    ...
    private Paudbunl(PautSummaryRepository summary, PautDetailRepository detail, IClock clock, HostKind host)
    {
        _summary = summary;
        _detail = detail;
        _clock = clock;
    ...
    public static PaudbunlResult Run(
    ...
    {
        var program = new Paudbunl(summary, detail, clock ?? SystemClock.Instance, host);
        program.MainPara(outfil1Path, outfil2Path);
        return new PaudbunlResult(program.Sysout, program.ReturnCode);
    }
...
/// <summary>
/// The result of a <see cref="Paudbunl"/> run: the SYSOUT (DISPLAY) lines in order and the batch
...
public sealed record PaudbunlResult(IReadOnlyList<string> Sysout, int ReturnCode);
