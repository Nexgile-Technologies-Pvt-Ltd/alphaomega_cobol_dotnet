...
/// <c>PAUTDTL1</c> child. The input record images are byte-identical to <see cref="Paudbunl"/>'s output, so
...
public sealed class Paudblod
{
    ...
    private Paudblod(PautSummaryRepository summary, PautDetailRepository detail, IClock clock, HostKind host)
    {
        _summary = summary;
        _detail = detail;
        _clock = clock;
    ...
    /// holds the 206-byte (root-key + detail) records — exactly what <see cref="Paudbunl"/> writes.
    /// </summary>
    ...
    public static PaudblodResult Run(
    ...
    {
        var program = new Paudblod(summary, detail, clock ?? SystemClock.Instance, host);
        program.MainPara(infile1Path, infile2Path);
        return new PaudblodResult(program.Sysout, program.ReturnCode);
    }
...
/// <summary>
/// The result of a <see cref="Paudblod"/> run: the SYSOUT (DISPLAY) lines in order and the batch
...
public sealed record PaudblodResult(IReadOnlyList<string> Sysout, int ReturnCode);
