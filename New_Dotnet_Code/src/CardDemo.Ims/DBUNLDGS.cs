...
/// pending-authorization IMS DB into two GSAM files). It is the GSAM-output twin of <see cref="Paudbunl"/>:
...
/// treated as two sequential output files using the SAME segment images as <see cref="Paudbunl"/>: the
...
public sealed class Dbunldgs
{
    ...
    private Dbunldgs(PautSummaryRepository summary, PautDetailRepository detail, IClock clock, HostKind host)
    {
        _summary = summary;
        _detail = detail;
        _clock = clock;
    ...
    public static DbunldgsResult Run(
    ...
    {
        var program = new Dbunldgs(summary, detail, clock ?? SystemClock.Instance, host);
        program.MainPara(summaryGsamPath, detailGsamPath);
        return new DbunldgsResult(program.Sysout, program.ReturnCode);
    }
...
/// <summary>
/// The result of a <see cref="Dbunldgs"/> run: the SYSOUT (DISPLAY) lines in order and the batch
...
public sealed record DbunldgsResult(IReadOnlyList<string> Sysout, int ReturnCode);
